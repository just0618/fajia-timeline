...
const mayData = {
  ...
  events:[
    ...
    },
    {
      ...
      gallery:[{src:'media/2026/05/0518_tianjin_ferris_live/tianjin-vlog.png',caption:'摩天轮画面 · 后续天津碎片中公开'},{src:'media/2026/05/0518_tianjin_ferris_live/douyin-0522.png',caption:'05.22 · 天津相关抖音公开'},{src:'media/2026/05/0518_tianjin_ferris_live/weibo-0608.jpg',caption:'06.08 · 「一点天津的小碎片」完整微博'}],
  ...
  events:[
    {
      ...
      hero:{type:'image',src:'media/2026/06/0601_thesis_ack/source-weibo.jpg',caption:'06.01 · 贺嘉述毕业论文致谢微博'},gallery:[{src:'media/2026/06/0601_thesis_ack/thesis-page.jpg',caption:'毕业论文致谢页面'}],
...
function renderHero(h){if(h?.type==='image')return `<figure class="polaroid"><span class="photo-tape"></span><img src="${h.src}" alt="${esc(h.caption)}" loading="lazy" decoding="async"><figcaption>${esc(h.caption)}</figcaption></figure>`;return `<figure class="polaroid"><span class="photo-tape"></span><div class="fake-photo"><div><b>${esc(h?.stamp||'NOTE')}</b><span>${esc(h?.caption||'')}</span></div></div><figcaption>${esc(h?.caption||'placeholder')}</figcaption></figure>`;}
function renderGallery(g=[]){return !g.length?'':`<div class="gallery-strip">${g.map(i=>`<figure class="gallery-photo"><img src="${i.src}" alt="${esc(i.caption)}" loading="lazy" decoding="async"><figcaption>${esc(i.caption)}</figcaption></figure>`).join('')}</div>`;}
