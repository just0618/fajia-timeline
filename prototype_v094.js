...
const readerMonthMeta = {
  ...
  '2026-06':{
    ...
    title:'六月 · 事情开始有了前因后果',
    lead:'一些素材在六月发生，到了七月才被公开。源头与后来被看见的时刻，开始连成线。',
    note:'跨月关系保留，但默认不把考据过程铺满页面。'
  },
  '2026-07':{
    eyebrow:'CHAPTER 07 · JULY',
    title:'七月 · 工作与公开一页页展开',
...
const readerProfiles = {
  comment_20260503:{
    story:'05.03，两个人在法宣阁抖音评论区发生了一段公开互动。',
    note:'能确认：互动发生于 05.03。暂不确认：母帖影像的实际拍摄日。',
  },
  study_20260510:{
    story:'05.10，法宣阁发布微博“有人还在赶论文”。',
    note:'05.10 可以确认的是微博公开日期；画面的实际拍摄时间暂不额外推定。'
  },
  littlehero_20260514:{
    story:'05.14，法宣阁与贺嘉述分别发布《小英雄》相关微博宣传。',
    note:'这一页只记录《小英雄》宣传；同日的视频号直播保留为下一页的独立事件。'
  },
  live_20260514:{
    story:'05.14，两人进行了视频号双人直播。',
    note:'直播与《小英雄》宣传发生在同一天，但作为两个独立事件分别记录。'
  },
  thesis_ack_20260601:{
    story:'06.01，贺嘉述公开了毕业论文致谢相关内容，其中出现了对法宣阁的致谢。',
    note:'06.01 是公开日期；论文致谢实际写作时间早于公开，具体日期暂未确认。'
  },
  life_love_live_20260611:{
    story:'这晚先有“生命自由爱”直播。直播里又出现了“可以发一下”的提醒，于是同日有了微博和小红书；后来公开的露营日记素材，也能由服装确认拍于这一天。',
    note:'06.11 同时有直播、发布和素材拍摄三件不同的事。它们共享日期，但不是同一个时间事实。',
  },
  birthday_mall_20260718:{
    story:'这一天从生日应援现场开始：两人提前给粉丝买了约 50 杯奶茶，下午到了商场。结束后，在前往机场的路上短随播，晚上从北京前往杭州。',
    note:'手账只保留城市级动线；机场、航班与精确时刻不展示。',
  },
  car_live_20260718:{
    story:'生日应援现场结束后，两人在去机场的路上短随播了十几分钟。网络很卡，但这段短暂的公开互动本身仍然值得留下一页。',
    note:'只记录随播发生在 07.18，以及它处在“应援现场之后、前往机场途中”这个公开可知的背景里。'
  }
};

function readerFactualStory(e){
  const rows=(e.real_timeline||[]).slice(0,2);
  if(rows.length){
    return rows.map((row,idx)=>{
      const date=row.date||rangeLabel(e);
      const title=row.title||e.title;
      const raw=String(row.text||'').trim();
      const keep=raw && !/(节点|模板|页面|落位|验证|结构|公开可见|标准|记录。?$|PUBLIC POST|REAL EVENT)/i.test(raw);
      return `${date}，${title}${keep?`：${raw}`:''}。`;
    }).join(' ');
  }
  return `${rangeLabel(e)}，${e.title}。`;
}
...
function readerEssentialNote(e){
  ...
  if(e.evidence_mode==='later_public'){
    const later=(e.public_evidence||[])[0];
    if(later) return `这页以现实发生时间为主；${fmtMonthDay(later.published_at)||later.published_at} 的公开内容作为后来被看见的记录。`;
  }
  const first=(e.time_anchors||[])[0];
  return first?`${first.label}：${first.value}。${first.note||''}`:'这一页先保留当前能够确认的事实。';
}

function readerKind(e){
  if(/直播|live/i.test(`${e.title} ${e.layout_type}`)) return '直播';
  if(/拍摄|shoot/i.test(`${e.title} ${e.layout_type}`)) return '拍摄';
  if(e.node_type==='public_interaction') return '互动';
  if(e.node_type==='public_clue') return '线索';
  if(e.node_type==='public_post') return '公开记录';
  if(/行程|出行|散步|现场|演唱会|见面会/.test(e.title)) return '现实记录';
  return '手账记录';
}
  ...
  events:[
    {
      ...
      hero:{type:'image',src:'media/2026/05/0503_comment_interaction/comment-thread.jpg',caption:'05.03 · 评论区公开小互动'},gallery:[{src:'media/2026/05/0503_comment_interaction/mother-post.jpg',caption:'05.03 · 法宣阁抖音母帖（互动发生的上下文）'}],
    ...
    },
    {
      ...
      hero:{type:'image',src:'media/2026/05/0514_littlehero/post-fxg.jpg',caption:'05.14 · 法宣阁《小英雄》相关微博',fit:'contain'},
      gallery:[{src:'media/2026/05/0514_littlehero/post-hjs.jpg',caption:'05.14 · 贺嘉述《小英雄》相关微博',fit:'contain'}],
      left_intro:'05.14，法宣阁与贺嘉述分别发布《小英雄》相关微博宣传。',
      time_anchors:[{label:'公开发布时间',value:'05.14',note:'两人同日公开。',tone:'pink'}],
      fact_memos:[{type:'pink',title:'本页记录：',text:'《小英雄》双人宣传微博。'}],
      real_timeline:[{date:'05.14',title:'《小英雄》双人宣传',text:'两人分别发布相关微博。'}],
      left_note:'Little Hero ♫',
    ...
    },
    {
      ...
      gallery:[{src:'media/2026/05/0518_tianjin_ferris_live/tianjin-vlog.png',caption:'摩天轮画面 · 后续天津碎片中公开',fit:'contain'},{src:'media/2026/05/0518_tianjin_ferris_live/douyin-0522.png',caption:'05.22 · 天津相关抖音公开',fit:'contain'},{src:'media/2026/05/0518_tianjin_ferris_live/weibo-0608.jpg',caption:'06.08 · 「一点天津的小碎片」完整微博',fit:'contain'}],
      left_intro:'现实发生留在 05.18–19，后来公开的天津内容再挂回这一页。',
      time_anchors:[{label:'现实发生时间',value:'05.18–05.19',note:'天津主事件与城市级动线。',tone:'pink'},{label:'后续公开',value:'05.22 / 05.27 / 06.08',note:'天津阶段素材在之后分批公开。',tone:'yellow'}],
      ...
      public_evidence:[{published_at:'2026-05-22',platform:'DOUYIN',title:'怎么哄？在线等!急!',relation:'same Tianjin period',text:'天津阶段拍摄素材，05.22 后续公开。',url:'',art:'video'},{published_at:'2026-05-27',platform:'DOUYIN',title:'闪现一下⚡',relation:'same Tianjin period',text:'素材发生于天津阶段，05.27 后续公开。当前素材包中尚未补入该条完整截图。',url:'',art:'video'},{published_at:'2026-06-08',platform:'WEIBO',title:'一点天津的小碎片',relation:'retrospective',text:'作为后续公开材料挂回天津页。',url:'',art:'postcard'}],
    ...
    },
    {
      ...
      hero:{type:'image',src:'media/2026/05/0524_musicfestival/duo-01.JPG',caption:'05.24 · 大眼音乐节双人合照'},
      gallery:[{src:'media/2026/05/0524_musicfestival/duo-02.JPG',caption:'05.24 · 音乐节双人合照'},{src:'media/2026/05/0524_musicfestival/stage.JPG',caption:'05.24 · 大眼音乐节舞台'},{src:'media/2026/05/0524_musicfestival/weibo-fxg.jpg',caption:'法宣阁 05.24 音乐节微博'},{src:'media/2026/05/0524_musicfestival/weibo-hjs.jpg',caption:'贺嘉述 05.24 音乐节微博'}],
  ...
  events:[
    {
      ...
      hero:{type:'image',src:'media/2026/06/0601_thesis_ack/source-weibo.jpg',caption:'06.01 · 贺嘉述毕业论文致谢微博',fit:'contain'},gallery:[{src:'media/2026/06/0601_thesis_ack/thesis-page.jpg',caption:'毕业论文致谢页面',fit:'contain'}],
      left_intro:'06.01，贺嘉述公开毕业论文致谢相关内容，其中出现了对法宣阁的致谢。',
    ...
    },
    {
      ...
      hero:{type:'image',src:'media/2026/06/0625-27_qingdao_shoot/polaroid-01.jpg',caption:'06.25–27 · 青岛拍摄拍立得'},gallery:[{src:'media/2026/06/0625-27_qingdao_shoot/polaroid-02.jpg',caption:'青岛拍摄拍立得 02'},{src:'media/2026/06/0625-27_qingdao_shoot/polaroid-03.JPG',caption:'青岛拍摄拍立得 03'},{src:'media/2026/06/0625-27_qingdao_shoot/polaroid-04.JPG',caption:'青岛拍摄拍立得 04'},{src:'media/2026/06/0625-27_qingdao_shoot/fxg-0627-source.jpg',caption:'06.27 · 青岛小短片拍摄周期相关公开'}],
...
const julyData = {
  ...
  events:[
    {key:'home_live_20260702',node_type:'real_event',chapter:'JUL 01A',layout_type:'live_spread',evidence_mode:'same_day_public',title:'双人居家直播',short_title:'居家直播',tagline:'同日节点 ① · 直播',priority:'P0',range:[2],year:2026,status:'confirmed',date_precision:'exact',confidence:'confirmed',hero:{type:'image',src:'media/2026/07/0702_home_live/cover.JPG',caption:'07.02 · 双人居家直播'},gallery:[{src:'media/2026/07/0702_home_live/gallery-01.JPG',caption:'07.02 · 直播画面 02'}],left_intro:'七月第一个已确认的直播现实节点。',time_anchors:[{label:'直播发生时间',value:'07.02',note:'日期确认。',tone:'pink'}],fact_memos:[{type:'pink',title:'节点性质：',text:'REAL EVENT / 直播。'}],real_timeline:[{date:'07.02',title:'双人居家直播',text:'现实节点确认。'}],left_note:'July starts here ✦',public_evidence:[{published_at:'2026-07-02',platform:'LIVE',title:'直播同步公开',relation:'same-day public',text:'直播同步可见。',art:'video'}],reading_logic:'标准直播节点。'},
    ...
    {key:'snack_live_20260710',node_type:'real_event',chapter:'JUL 08A',layout_type:'live_spread',evidence_mode:'same_day_public',title:'双人直播 · 零食局',short_title:'零食局直播',tagline:'同日节点 ① · 现实直播',priority:'P0',range:[10],year:2026,status:'confirmed',date_precision:'exact',confidence:'confirmed',hero:{type:'image',src:'media/2026/07/0710_snack_live/cover.JPG',caption:'07.10 · 双人直播'},left_intro:'标准的同日直播现实节点。',time_anchors:[{label:'直播时间',value:'07.10',note:'confirmed。',tone:'pink'}],real_timeline:[{date:'07.10',title:'双人直播',text:'零食局。'}],left_note:'snack live ✦',public_evidence:[{published_at:'2026-07-10',platform:'LIVE',title:'零食局直播',relation:'same-day public',text:'直播同步可见。',art:'video'}],reading_logic:'直播本体与同日见面会官宣并列。'},
    ...
    {key:'fe_live_20260719',node_type:'real_event',chapter:'JUL 15A',layout_type:'brand_live_spread',evidence_mode:'same_day_public',title:'FunnyElves 方里直播',short_title:'方里直播',tagline:'同日节点 ① · 商务直播',priority:'P0',range:[19],year:2026,status:'confirmed',date_precision:'exact',confidence:'confirmed',hero:{type:'image',src:'media/2026/07/0719_funnyelves_live/cover.JPG',caption:'07.19 · FunnyElves 方里直播'},gallery:[{src:'media/2026/07/0719_funnyelves_live/fxg-weibo.jpg',caption:'07.19 · 法宣阁方里直播相关微博'}],left_intro:'标准商务直播现实节点。',time_anchors:[{label:'直播时间',value:'07.19',note:'confirmed。',tone:'pink'}],real_timeline:[{date:'07.19',title:'方里直播',text:'商务直播。'}],left_note:'brand live ✦',public_evidence:[{published_at:'2026-07-19',platform:'FUNNYELVES LIVE',title:'方里直播',relation:'same-day public',text:'直播同步公开。',art:'video'}],reading_logic:'与同日多条公开内容并列。'},
    ...
    {key:'tie_posts_20260719',node_type:'public_post',chapter:'JUL 15C',layout_type:'paired_post_spread',evidence_mode:'same_day_public',title:'两人「贴贴」微博',short_title:'贴贴微博',tagline:'同日节点 ③ · 当日拍 / 当日发',priority:'P0',range:[19],year:2026,status:'confirmed',date_precision:'exact',confidence:'confirmed',hero:{type:'image',src:'media/2026/07/0719_tie_posts/hjs-weibo.jpg',caption:'07.19 · 贺嘉述同日微博'},left_intro:'这组微博判断为当天拍摄、当天发布，因此可以把拍摄与公开都放在 07.19。',time_anchors:[{label:'拍摄时间',value:'07.19',note:'人工核对判断 confirmed。',tone:'yellow'},{label:'公开时间',value:'07.19',note:'same-day。',tone:'pink'}],real_timeline:[{date:'07.19',title:'拍摄并公开',text:'同日节点。'}],left_note:'same-day post ♡',public_evidence:[{published_at:'2026-07-19',platform:'WEIBO · 双人',title:'“被”贴贴 / 贴贴“你”',relation:'same-day public',text:'当日拍摄并发布。',art:'postcard'}],reading_logic:'能确认同日拍摄时，就不需要保留 unknown。'},
    ...
    {key:'failed_live_20260722',node_type:'public_interaction',chapter:'JUL 18',layout_type:'interaction_spread',evidence_mode:'same_day_public',title:'随播失败版 · 网络太卡',short_title:'失败随播',tagline:'公开互动 · 直播未能继续',priority:'P0',range:[22],year:2026,status:'confirmed',date_precision:'exact',confidence:'confirmed',hero:{type:'image',src:'media/2026/07/0722_failed_live/cover.JPG',caption:'07.22 · 网络卡顿的随播画面'},left_intro:'首饰盒 01:56 明确说明本想聊天，但因网络太卡无法继续。',time_anchors:[{label:'发生 / 说明时间',value:'07.22 01:56',note:'confirmed。',tone:'pink'}],real_timeline:[{date:'07.22',title:'短随播尝试失败',text:'网络原因无法继续。'}],left_note:'failed live is still a node ✦',public_evidence:[{published_at:'2026-07-22',platform:'小发夹的首饰盒',title:'本想跟大家聊聊天…',relation:'interaction proof',text:'网络太卡导致无法继续。',art:'postcard'}],reading_logic:'未成功完成的随播仍然是一段真实发生的公开互动。'},
    {key:'birthday_live_20260723_24',node_type:'real_event',chapter:'JUL 19',layout_type:'cross_midnight_spread',evidence_mode:'same_day_public',title:'法宣阁生日双人直播 / DR 礼物',short_title:'生日直播',tagline:'07.23 evening → 07.24 early AM',priority:'P0',range:[23,24],year:2026,status:'confirmed',date_precision:'range',confidence:'confirmed',hero:{type:'image',src:'media/2026/07/0723_birthday_live/cover.JPG',caption:'07.23 · 生日双人直播'},gallery:[{src:'media/2026/07/0723_birthday_live/dr.JPG',caption:'07.24 凌晨 · DR 礼物'},{src:'media/2026/07/0723_birthday_live/fxg-birthday.jpg',caption:'07.23 · 法宣阁生日微博'},{src:'media/2026/07/0723_birthday_live/hjs-birthday.jpg',caption:'07.23 · 贺嘉述庆生微博'}],left_intro:'生日直播跨过午夜，DR 礼物发生在 07.24 凌晨，因此不把所有事情压回 07.23。',time_anchors:[{label:'直播开始',value:'07.23 evening',note:'生日直播。',tone:'pink'},{label:'DR 礼物',value:'07.24 early AM',note:'跨午夜后的节点。',tone:'yellow'}],real_timeline:[{date:'07.23',title:'生日双人直播',text:'晚间开始。'},{date:'07.24 凌晨',title:'DR 礼物',text:'跨午夜发生。'}],left_note:'one night across two dates ♡',public_evidence:[{published_at:'2026-07-23',platform:'LIVE',title:'生日双人直播',relation:'cross midnight',text:'跨午夜现实节点。',art:'video'}],reading_logic:'跨午夜事件可以占据两个日期，但仍保持一个章节。'},
    ...
    {key:'concert_20260725',node_type:'real_event',chapter:'JUL 21A',layout_type:'stage_spread',evidence_mode:'same_day_public',title:'「宿命回响·双程」曼谷演唱会',short_title:'曼谷演唱会',tagline:'同日节点 ① · 现实舞台',priority:'P0',range:[25],year:2026,status:'confirmed',date_precision:'exact',confidence:'confirmed',hero:{type:'image',src:'media/2026/07/0725_concert/hjs-stage-post.jpg',caption:'07.25 · 曼谷演唱会舞台相关公开'},left_intro:'七月的大型现实舞台节点。',time_anchors:[{label:'活动日期',value:'07.25',note:'confirmed。',tone:'pink'},{label:'地点',value:'曼谷',note:'城市级。',tone:'yellow'}],real_timeline:[{date:'07.25',title:'曼谷演唱会',text:'现实活动。'}],left_note:'Bangkok stage ✦',public_evidence:[{published_at:'2026-07-25',platform:'LIVE EVENT',title:'宿命回响·双程曼谷演唱会',relation:'same-day public',text:'现实活动公开可见。',art:'video'}],reading_logic:'官宣在 07.06，现实活动本体在 07.25。'},
    {key:'concert_posts_20260725',node_type:'public_post',chapter:'JUL 21B',layout_type:'paired_post_spread',evidence_mode:'same_day_public',title:'两人出发图 / 首饰盒「红玫瑰和黑西装」',short_title:'演唱会当日公开',tagline:'同日节点 ② · 舞台相关发布',priority:'P0',range:[25],year:2026,status:'confirmed',date_precision:'exact',confidence:'confirmed',hero:{type:'image',src:'media/2026/07/0725_concert/fxg-departure.jpg',caption:'07.25 · 法宣阁演唱会出发图'},gallery:[{src:'media/2026/07/0725_concert/hjs-stage-post.jpg',caption:'07.25 · 贺嘉述演唱会相关微博'}],left_intro:'活动当天还有两人出发图与首饰盒“红玫瑰和黑西装”等公开内容。',time_anchors:[{label:'公开时间',value:'07.25',note:'活动同日。',tone:'pink'}],real_timeline:[{date:'07.25',title:'舞台相关公开内容',text:'与现实演唱会同日并列。'}],left_note:'stage day posts ✦',public_evidence:[{published_at:'2026-07-25',platform:'WEIBO / 首饰盒',title:'出发图 / 红玫瑰和黑西装',relation:'same-day public',text:'活动同日公开。',art:'postcard'}],reading_logic:'现实活动与当日公开内容保持多节点。'},
    {key:'meeting_20260726',node_type:'real_event',chapter:'JUL 22',layout_type:'event_spread',evidence_mode:'same_day_public',title:'「宣你述说」曼谷双人见面会',short_title:'曼谷见面会',tagline:'最终 13:00 · Lido Connect Hall 3',priority:'P0',range:[26],year:2026,status:'confirmed',date_precision:'exact',confidence:'confirmed',hero:{type:'image',src:'media/2026/07/0726_meeting/fxg-weibo.jpg',caption:'07.26 · 法宣阁曼谷见面会相关微博'},gallery:[{src:'media/2026/07/0726_meeting/hjs-weibo.jpg',caption:'07.26 · 贺嘉述曼谷见面会相关微博'}],left_intro:'现实活动在 07.26 举行；07.20 的时间 / 场地变更公告作为 event_update 指向这里。',time_anchors:[{label:'活动日期',value:'07.26',note:'confirmed。',tone:'pink'},{label:'最终信息',value:'13:00 · Lido Connect Hall 3',note:'保存变更后的版本。',tone:'yellow'}],real_timeline:[{date:'07.20',title:'event update',text:'场地 / 时间变更。',soft:true,tag:'UPDATE'},{date:'07.26',title:'曼谷双人见面会',text:'现实活动。'}],left_note:'updated, then met ✦',public_evidence:[{published_at:'2026-07-26',platform:'LIVE EVENT',title:'宣你述说双人见面会',relation:'event after update',text:'现实活动。',art:'video'}],reading_logic:'07.20 只更新信息，不创建第二个活动。'},
...
function renderMonthHeader(){
  const data=currentData(),m=data.meta,r=readerMonthMeta[currentMonthKey]||{};
  $('#monthNum').textContent=m.num;$('#monthName').textContent=m.name;$('#monthYear').textContent=m.year;
  $('#monthLead').textContent=r.lead||m.lead;
  $('#monthRule').textContent=r.note||m.rule;
  $('#cornerNote').textContent=`${m.name} notes ✎`;
  $('#monthRightTitle').textContent=r.title||m.rightTitle;
  $('#monthIntroTitle').textContent='这一章，先从故事开始。';
  $('#monthIntroText').textContent=r.lead||m.introText;
  $('#chapterTitle').textContent=`${m.name} 的手账目录`;
  ...
  if(idx<monthOrder.length-1){const next=monthOrder[idx+1];turn.classList.remove('hidden');turn.textContent=`翻到下一章 · ${monthStore[next].meta.name} →`;turn.onclick=()=>switchMonth(next);} else turn.classList.add('hidden');
  renderMonthSpecial();
  renderMobileMonthReader();
}

function renderMonthSpecial(){
  const box=$('#monthSpecial'),r=readerMonthMeta[currentMonthKey]||{};
...
function switchMonth(key){
  ...
  document.body.classList.remove('show-mobile-calendar');
...
function renderCalendar(){
  ...
  for(let d=1;d<=daysInMonth(year,month);d++){
    ...
    if(matches.length===1){
      ...
      const status=(e.status==='draft'||e.date_precision==='inferred')?`<span class="status-badge">${e.date_precision==='inferred'?'推定':'待确认'}</span>`:'';
...
function renderChapterRail(){
  const data=currentData();
  $('#chapterRail').innerHTML=data.events.map(e=>{const rp=readerProfile(e);return `<button class="chapter-card reader-card ${e.key===currentEventKey?'active':''}" data-event="${e.key}"><div class="chapter-badge"><div><b>${rangeLabel(e)}</b><small>${data.meta.name.toUpperCase()} ${data.meta.year}</small></div></div><small class="reader-kind">${esc(readerKind(e))}</small><h4>${esc(e.title)}</h4><p>${esc(rp.story)}</p><span class="view-arrow">翻到这一页 →</span></button>`;}).join('');
...
function renderReaderPeers(e){
  const peers=sameDayNeighbors(e); if(!peers.length)return '';
  return `<section class="reader-peers"><small>同一天，还有</small><div>${peers.map(p=>`<button data-peer="${p.key}"><span>${esc(readerKind(p))}</span><b>${esc(p.short_title)}</b></button>`).join('')}</div></section>`;
}
...
function renderReaderRelations(e){
  ...
  return parts.length?`<section class="reader-relations">${parts.join('')}</section>`:'';
}
...
// QA/deep-link helper: useful for previewing a specific month/event without extra clicks.
// It stays dormant during normal use.
const __previewParams=new URLSearchParams(location.search);
if(__previewParams.get('preview')){
  const mk=__previewParams.get('month');
  const ev=__previewParams.get('event');
  if(mk&&monthStore[mk]) currentMonthKey=mk;
  currentEventKey=(ev&&findEvent(ev))?ev:(currentData().events[0]?.key||'');
